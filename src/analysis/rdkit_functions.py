...
class BasicMolecularMetrics(object):
    ...
    def compute_novelty(self, unique):
        ...
        for smiles in unique:
            if smiles not in self.dataset_smiles_list:
                ...
                print("Matched smile:{}".format(smiles))
    ...
    def compare_datasets_fcd_score(self, smiles_list_1, smiles_list_2, device):
        fcd = FCD(device=device, n_jobs=8)
        return fcd(smiles_list_1, smiles_list_2)
    
    def compare_datasets_fcd_score1(self, smiles_list_1, smiles_list_2, device):
        from fcd import get_fcd, load_ref_model,canonical_smiles, get_predictions, calculate_frechet_distance
        fcd_model = load_ref_model()
        fcd_score = get_fcd(smiles_list_1, smiles_list_2, fcd_model)
    ...
    def evaluate(self, generated):
        ...
        if relaxed_validity > 0:
            ...
            if self.dataset_smiles_list is not None:
                _, novelty = self.compute_novelty(relaxed_valid)
                recon_rate = (len(relaxed_valid) * (1-novelty))/len(generated)
                print(f"Novelty over {len(relaxed_valid)} valid molecules: {novelty * 100 :.2f}%")
                print(f"Recon rate over {len(generated)} molecules: {recon_rate * 100 :.2f}%")
                # print("Train smiles:", self.dataset_smiles_list.tolist())
                print("ALL smiles:", all_smiles)
                print("valid smiles:", valid)
                '''FCD '''
                try:
                    fcd_score = self.compare_datasets_fcd_score(valid, self.dataset_smiles_list.tolist(), torch.device('cuda:0'))
                except:
                    print("error all")
                    fcd_score = -1.0
                # print(f"FCD score: {fcd_score :.4f}%")
                try:
                    relax_fcd_score = self.compare_datasets_fcd_score(relaxed_valid, self.dataset_smiles_list.tolist(), torch.device('cuda:0'))
                except:
                    print("relax error")
                    relax_fcd_score = -1.0
                print(f"FCD score: {fcd_score :.4f}% Relax FCD score: {relax_fcd_score :.4f}%")

                try:
                    fcd_score = self.compare_datasets_fcd_score1(all_smiles, self.dataset_smiles_list.tolist(), torch.device('cuda:0'))
                except:
                    print("error all")
                    fcd_score = -1.0
                # print(f"FCD score: {fcd_score :.4f}%")
                try:
                    relax_fcd_score = self.compare_datasets_fcd_score1(all_smiles, self.dataset_smiles_list.tolist(), torch.device('cuda:0'))
                except:
                    print("relax error")
                    relax_fcd_score = -1.0
                print(f"FCD score1: {fcd_score :.4f}% Relax FCD score1: {relax_fcd_score :.4f}%")
            else:
                novelty = -1.0
                fcd_score = -1.0
        else:
            ...
            unique = []
            fcd_score = -1.0
        return ([validity, relaxed_validity, uniqueness, novelty, fcd_score], unique,
...
def build_molecule(atom_types, edge_types, atom_decoder, verbose=False):
    ...
    for i, bond in enumerate(all_bonds):
        if bond[0].item() != bond[1].item():
            ...
            mol.AddBond(bond[0].item(), bond[1].item(), Chem.rdchem.BondType.SINGLE)
            if verbose:
        ...
        if flag:
            ...
            if queue[-1][1] == 12:
                ...
                if t >= 1:
                    ...
                    mol.AddBond(start, end, Chem.rdchem.BondType.SINGLE)
...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info):
    ...
    if wandb.run:
        ...
        dic = {'Validity': rdkit_metrics[0][0], 'Relaxed Validity': rdkit_metrics[0][1],
               'Uniqueness': rdkit_metrics[0][2], 'Novelty': rdkit_metrics[0][3], 'FCD Score': rdkit_metrics[0][4],
