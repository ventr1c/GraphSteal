...
class ExtraMolecularFeatures:
    def __init__(self, dataset_infos):
        if(dataset_infos.name == 'zinc'):
            self.charge = ChargeFeatureZINC(remove_h=dataset_infos.remove_h, valencies=dataset_infos.valencies)
            self.valency = ValencyFeatureZINC()
        else:
...
class ChargeFeatureZINC:
    def __init__(self, remove_h, valencies):
        self.remove_h = remove_h
        self.valencies = valencies

    def __call__(self, noisy_data):
        bond_orders = torch.tensor([1, 2, 3], device=noisy_data['E_t'].device).reshape(1, 1, 1, -1)
        weighted_E = noisy_data['E_t'] * bond_orders      # (bs, n, n, de)
        current_valencies = weighted_E.argmax(dim=-1).sum(dim=-1)   # (bs, n)
...
class ValencyFeatureZINC:
    def __init__(self):
        pass

    def __call__(self, noisy_data):
        orders = torch.tensor([1, 2, 3], device=noisy_data['E_t'].device).reshape(1, 1, 1, -1)
        E = noisy_data['E_t'] * orders      # (bs, n, n, de)
        valencies = E.argmax(dim=-1).sum(dim=-1)    # (bs, n)
        return valencies.type_as(noisy_data['X_t'])
