...
def partial_sample(probVar):
    is_nonzero = probVar.sum(dim=1) > 0  # Identify non-zero rows
    nonzero_probVar = probVar[is_nonzero]  # Filter out non-zero rows
    nonzero_probVar = F.relu(nonzero_probVar)
    ## Perform sampling on non-zero rows only
    nonzero_V_t = nonzero_probVar.multinomial(1) if len(nonzero_probVar) > 0 else torch.tensor([], dtype=torch.long)

    ## Prepare a tensor to store the final result
    V_t = torch.zeros_like(probVar[:, 0:1]).long()

    ## Place the sampled values back into the corresponding positions
    V_t[is_nonzero] = nonzero_V_t

    return V_t

def sample_discrete_features(probX, probE, node_mask, device = None):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        :param probX: bs, n, dx_out        node features
        :param probE: bs, n, n, de_out     edge features
        :param proby: bs, dy_out           global features.
    '''
    probX = probX.to(device)
...
def sample_discrete_feature_noise_with_y(limit_dist, node_mask, num_classes, sample_y):
    """ Sample from the limit distribution of the diffusion process"""
    bs, n_max = node_mask.shape
    x_limit = limit_dist.X[None, None, :].expand(bs, n_max, -1)
    e_limit = limit_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
    ...
    U_E = e_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)
    U_y = torch.empty((bs, 0))
    # U_y = F.one_hot(sample_y, num_classes = num_classes)
...
def initialize_sample_discrete_feature_noise_with_y(limit_dist, node_mask, num_classes, sample_y, device):
    """ Sample from the limit distribution of the diffusion process"""
    bs, n_max = node_mask.shape
    x_limit = limit_dist.X[None, None, :].expand(bs, n_max, -1)
    e_limit = limit_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
    ...
    x_limit = Parameter(x_limit).to(device)
    e_limit = Parameter(e_limit).to(device)
    print("x_limit",x_limit.shape)
    print('e_limit',e_limit.shape)
    U_X = x_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max)
    U_E = e_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)
    U_y = torch.empty((bs, 0))
    # U_y = F.one_hot(sample_y, num_classes = num_classes)
    ...
    U_X = Parameter(U_X).to(device)
    U_E = Parameter(U_E).to(device)
    U_y = U_y.to(device)
    print('U_y', U_y)
    return PlaceHolder(X=U_X, E=U_E, y=U_y).mask(node_mask)
...
def initialize_limit(limit_dist, node_mask, device):
    """ Sample from the limit distribution of the diffusion process"""
    bs, n_max = node_mask.shape
    x_limit = limit_dist.X[None, None, :].expand(bs, n_max, -1)
    e_limit = limit_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
...
def sample_discrete_feature_noise_with_limit(x_limit, e_limit, node_mask, num_classes, sample_y, device):
    """ Sample from the limit distribution of the diffusion process"""
    bs, n_max = node_mask.shape
    # x_limit = limit_dist.X[None, None, :].expand(bs, n_max, -1)
    # e_limit = limit_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
    ...
    x_limit = x_limit.to(device)
    e_limit = e_limit.to(device)
    print("x_limit",x_limit.shape)
    print('e_limit',e_limit.shape)
    U_X = x_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max)
    U_E = e_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)
