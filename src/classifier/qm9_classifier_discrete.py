...
class Qm9ClassifierDiscrete(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.train_loss = F.cross_entropy
        self.val_loss = F.cross_entropy
        self.test_loss = F.cross_entropy
        self.test_acc = Accuracy(task="multiclass", num_classes=cfg.classifier.num_classes, top_k=1)

        self.best_val_mae = 1e8

        # self.val_loss_each = [F.cross_entropy.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu')) for i
        #                       in range(2)]
        # self.test_loss_each = [F.cross_entropy.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu')) for
        #                        i in range(2)]
        self.target_dict = {0: "mu", 1: "homo"}

        self.validation_step_outputs = []
        self.best_val_ce = 1e10

        self.train_step_outputs = []
        self.best_train_ce = 1e10
    def training_step(self, data, i):
        ...
        # y = label_regression2classification(target, self.args.classifier.num_classes, torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        loss_ce = self.compute_train_loss(pred.y, target, log=i % self.log_every_steps == 0)
        self.train_step_outputs.append(loss_ce)
        return {'loss': loss_ce}

    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.args.classifier.train_lr, amsgrad=True, weight_decay=1e-12)
    ...
    def on_train_epoch_end(self) -> None:
        ...
        train_ce = self.train_step_outputs[-1]
        self.train_step_outputs.clear()

        to_log = {"train_epoch/ce": train_ce}
        print(f"Epoch {self.current_epoch}: train_ce: {train_ce :.3f} -- {time.time() - self.start_epoch_time:.1f}s ")
    ...
    def validation_step(self, data, i):
        ...
        # y = label_regression2classification(target, self.args.classifier.num_classes, torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        loss_ce = self.compute_train_loss(pred.y, target, log=i % self.log_every_steps == 0)
        self.log('val_loss', loss_ce, prog_bar=True, on_step=False, on_epoch=True)
        self.validation_step_outputs.append(loss_ce)
        return {'loss': loss_ce}

    def on_validation_epoch_end(self) -> None:
        val_ce = self.validation_step_outputs[-1]
        # val_mae = self.val_loss.compute()
        to_log = {"val/epoch_ce": val_ce}
        print(f"Epoch {self.current_epoch}: val_ce: {val_ce :.3f}")
        wandb.log(to_log)
        self.log('val/epoch_ce', val_ce, on_epoch=True, on_step=False)
        ...
        #     for i in range(2):
        #         ce_each = self.val_loss_each[i].compute()
        #         print(f"Target {self.target_dict[i]}: val_ce: {ce_each :.3f}")
        #         to_log_each = {f"val_epoch/{self.target_dict[i]}_ce": ce_each}
    ...
    def compute_extra_data(self, noisy_data):
        ...
        # print('extra X',extra_features.X.shape)
        # print('extra E',extra_features.E.shape)
        # print('extra y',extra_features.y.shape)

        extra_X = torch.cat((extra_features.X, extra_molecular_features.X), dim=-1)
        extra_E = torch.cat((extra_features.E, extra_molecular_features.E), dim=-1)
        
        t= noisy_data['t']
        # t = torch.zeros(noisy_data['t'].shape[0], 0).type_as(noisy_data['t'])
    ...
    def compute_train_loss(self, pred, target, log: bool):
       ...
       """
        ...
        ce = self.train_loss(pred, target)

        if log:
            wandb.log({"train_loss/batch_ce": ce.item()}, commit=True)
        return ce
    ...
    def on_test_epoch_end(self) -> None:
        ...
        to_log = {"test_epoch/acc": test_acc}
        wandb.log(to_log)
        # self.train_loss.reset()
    ...
    def test_step(self, data, i):
        ...
        # mse = self.compute_train_loss(pred, target, log=i % self.log_every_steps == 0)
        # y = label_regression2classification(target, self.args.classifier.num_classes, torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        test_acc = self.test_acc(pred.y, target)
        self.log('test_acc', test_acc, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        return {'test acc': test_acc}
        # loss_ce = self.compute_train_loss(pred.y, y, log=i % self.log_every_steps == 0)
        # self.train_step_outputs.append(loss_ce)
        # return {'loss': loss_ce}