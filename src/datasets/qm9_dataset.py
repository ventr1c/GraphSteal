...
class QM9Dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        # # convert to classification setting
        # tar_task = 0
        # new_data_list = []
        # regression_values = [data.y[:, tar_task] for data in data_list]
        # all_values = torch.cat(regression_values, dim=0)
        # quantiles = all_values.quantile(q=torch.tensor([0.25, 0.5, 0.75]))
        # thresholds = quantiles.tolist()
        # for data in tar_dataset:
        #     value = data.y[:, tar_task].item()
        ...
        #     new_data = Data(x=data.x, edge_index=data.edge_index, edge_attr=data.edge_attr, y=torch.tensor([class_label], pos=data.pos, z=data.z, name = data.name, idx = data.idx, dtype=torch.long))
...
class QM9DataModule(MolecularDataModule):
    def __init__(self, cfg, regressor: bool = False):
        ...
        datasets = {'train': QM9Dataset(stage='test', root=root_path, remove_h=cfg.dataset.remove_h,
                                        target_prop=target, pre_transform=convert_regression_to_categorial_qm9(3)),    # RemoveYTransform()
                    'val': QM9Dataset(stage='val', root=root_path, remove_h=cfg.dataset.remove_h,
                                      target_prop=target, pre_transform=convert_regression_to_categorial_qm9(3)),  # RemoveYTransform()
                    'test': QM9Dataset(stage='train', root=root_path, remove_h=cfg.dataset.remove_h,
                                       target_prop=target, pre_transform=convert_regression_to_categorial_qm9(3))} # transform
        # if(mode == 'Diffusion'):
        #     datasets = {'train': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_train_dataset_cluster71_2.pt'),    # RemoveYTransform()
        #                 'val': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_val_dataset_cluster71_2.pt'),  # RemoveYTransform()
        #                 'test': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_test_dataset_cluster71_2.pt')} # transform
        # elif(mode == 'Reconstruction'):
        #     datasets = {'test': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_train_dataset_cluster71_2.pt'),    # RemoveYTransform()
        #                 'val': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_val_dataset_cluster71_2.pt'),  # RemoveYTransform()
        #                 'train': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_test_dataset_cluster71_2.pt')} # transform
        # else:
        #     raise NotImplementedError
        # mode = 'Reconstruction' # 'Diffusion' or 'Reconstruction' 
        # if(mode == 'Diffusion'):
        #     datasets = {'train': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_train_dataset_cluster71_2.pt'),    # RemoveYTransform()
        #                 'val': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_val_dataset_cluster71_2.pt'),  # RemoveYTransform()
        #                 'test': torch.load('/home/mfl5681/project-training-graph-stealing/DiGress/src/distribution_shift_aux_test_dataset_cluster71_2.pt')} # transform
...
def get_train_smiles(cfg, train_dataloader, dataset_infos, evaluate_dataset=False):
    ...
    mode = 'Reconstruction'
    if(mode == 'Diffusion'):
        smiles_file_name = 'train_smiles_no_h_cluster71_2.npy' if remove_h else 'train_smiles_h.npy' # 'train_smiles_no_h_cluster71_2.npy'
    elif(mode == 'Reconstruction'):
        smiles_file_name = 'test_smiles_no_h_cluster71_2.npy' if remove_h else 'test_smiles_h.npy' # 'train_smiles_no_h_cluster71_2.npy'
...
def get_test_val_smiles(cfg, train_dataloader, dataset_infos, split_flag, evaluate_dataset=False):
    '''
    split_flag: string ['test' or 'val']
    '''
    if evaluate_dataset:
        assert dataset_infos is not None, "If wanting to evaluate dataset, need to pass dataset_infos"
    datadir = cfg.dataset.datadir
    ...
    atom_decoder = dataset_infos.atom_decoder
    root_dir = pathlib.Path(os.path.realpath(__file__)).parents[2]
    smiles_file_name = '{}_smiles_no_h.npy'.format(split_flag) if remove_h else '{}_smiles_h.npy'.format()
    smiles_path = os.path.join(root_dir, datadir, smiles_file_name)
    if os.path.exists(smiles_path):
        print("Dataset smiles were found.")
        train_smiles = np.load(smiles_path)
...
class convert_regression_to_categorial_qm9:
    ...
    def __call__(self, data):
        ...
        * mask rate number of atom indices
        ...
        # thresholds = quantiles.tolist()
        thresholds = [-73.11074829101562, 2.426069974899292]   # for FreeSolv
        ...
        for i in range(self.num_classes):
            if(i == self.num_classes - 1):
                update_y[torch.where(value > thresholds[0])[0]] = i
            elif(i == 0):
                update_y[torch.where(value <= thresholds[0])[0]] = i
            # else:
            #     condition = torch.logical_and(value <= thresholds[0], value > thresholds[i-1])
    ...
    def __repr__(self):
        return '{}(num_classes={},)'.format(
            self.__class__.__name__, self.num_classes)