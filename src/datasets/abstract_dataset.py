...
class AbstractDatasetInfos:
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features, num_classes = None):
        if(num_classes == None):
            num_classes = 0
        ...
        self.input_dims = {'X': example_batch['x'].size(1),
                           ...
                           'y': 0 + 1}      # example_batch['y'].size(1) # + 1 due to time conditioning
        ex_extra_feat = extra_features(example_data)
        self.input_dims['X'] += ex_extra_feat.X.size(-1)
        self.input_dims['E'] += ex_extra_feat.E.size(-1)
        self.input_dims['y'] += ex_extra_feat.y.size(-1)

        print("input_features: X: {}".format(example_batch['x'].size(1)))
        print("input_features: E: {}".format(example_batch['edge_attr'].size(1)))
        print("input_features: y: {}".format(0))

        print("extra_features: E: {}".format(ex_extra_feat.E.size(-1)))
        print("extra_features: X: {}".format(ex_extra_feat.X.size(-1)))
        print("extra_features: y: {}".format(ex_extra_feat.y.size(-1)))
        ...
        self.output_dims = {'X': example_batch['x'].size(1),
                            'E': example_batch['edge_attr'].size(1),
                            'y': num_classes} # set 1 for conditional training, 0 for unconditional training
